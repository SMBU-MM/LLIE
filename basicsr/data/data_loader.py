...
def remove_background(image):
    # Assuming the input is a Tensor of shape [C, H, W]
    image = image.permute(1, 2, 0).numpy()  # Convert to HWC for processing
    _EPS = 1e-7
    rgb_max = np.max(image, axis=(0, 1))
    rgb_min = np.min(image, axis=(0, 1))
    image = (image - rgb_min) * rgb_max / (rgb_max - rgb_min + _EPS)
    image = torch.from_numpy(image).permute(2, 0, 1)  # Convert back to CHW
    return image


class Flare_Image_Loader:
    ...
    def add_flare_low(self, base_img_tensor,index):
        gamma = np.random.uniform(1.8, 2.2)
        adjust_gamma = RandomGammaCorrection(gamma)
        adjust_gamma_reverse = RandomGammaCorrection(1 / gamma)
        color_jitter = transforms.ColorJitter(brightness=(0.8, 3), hue=0.0)
        ...
        if self.transform_base is not None:
            base_img = adjust_gamma(base_img_tensor)
            # print(base_img)
            base_img = self.transform_base(base_img)
        else:
            base_img = adjust_gamma(base_img_tensor)
        ...
        flare_img = Image.open(flare_path)
        flare_img = transforms.ToTensor()(flare_img)
        flare_img = adjust_gamma(flare_img)

        if self.reflective_flag:
            reflective_path = random.choice(self.reflective_list)
            reflective_img = Image.open(reflective_path)
            reflective_img = transforms.ToTensor()(reflective_img)
            reflective_img = adjust_gamma(reflective_img)
            flare_img = torch.clamp(flare_img + reflective_img, min=0, max=1)
    ...
    def add_flare_high(self, base_img_tensor,index):
        gamma = np.random.uniform(1.8, 2.2)
        adjust_gamma = RandomGammaCorrection(gamma)
        adjust_gamma_reverse = RandomGammaCorrection(1 / gamma)
        color_jitter = transforms.ColorJitter(brightness=(0.8, 3), hue=0.0)
    ...
    def load_gt_flare(self, gt_name, gt_path):
        self.gt_flag = True
        gt_list = []
        [gt_list.extend(glob.glob(gt_path + '/*.' + e)) for e in self.ext]
        self.gt_name_list.append(gt_name)
        self.gt_dict[gt_name] = gt_list
        self.gt_list.extend(gt_list)
        len_gt_list = len(self.gt_dict[gt_name])