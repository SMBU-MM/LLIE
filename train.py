...
def parse_options(is_train=True):
    ...
    parser.add_argument(
        '--opt', type=str, default='Options/GCANet_retrain.yml', help='Path to option YAML file.')
    parser.add_argument(
        '--launcher',
...
def main():
    ...
    # training
    logger.info(
        f'Start training from epoch: {start_epoch}, iter: {current_iter}, id: {os.getpid()}')
    print(os.getpid())
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and (current_iter %
                ...
                print(opt['val'])
                current_metric = {'psnr': 0, 'ssim': 0, 'lpips': 1}
                ...
                print(best_metric)
                # log best metric
                b_psnr = best_metric['psnr']
                c_psnr = current_metric['psnr']
                if b_psnr < c_psnr:
                    best_metric['psnr'] = c_psnr
                    # save best model
                    best_metric['iter'] = current_iter
                    model.save_best(best_metric, 'psnr')
                if best_metric['ssim'] < current_metric['ssim']:
                    best_metric['ssim'] = current_metric['ssim']
                    # save best model
                    best_metric['iter'] = current_iter
                    model.save_best(best_metric, 'ssim')
                if best_metric['lpips'] == 0 or best_metric['lpips'] > current_metric['lpips']:
                    best_metric['lpips'] = current_metric['lpips']
                    # save best model
                    best_metric['iter'] = current_iter
                    model.save_best(best_metric, 'lpips')
                if tb_logger:
